...
class AddGameViewModel(private val repository: AuthRepository): ViewModel(){
    ...
    fun updateUiState(newGameUiState: AddGameUiState, event: AddGameUiEvent){
        ...
        when(event) {
            ...
            state = if(!titleResult.successful) newGameUiState.copy(titleErr = true) else newGameUiState.copy(titleErr = false)
            }

            is AddGameUiEvent.YearChanged -> {
                val yearResult = Validator.validateGameYear(newGameUiState.releaseYear)
                state = if(!yearResult.successful) newGameUiState.copy(yearErr = true) else newGameUiState.copy(yearErr = false)
            }

            is AddGameUiEvent.PublisherChanged -> {
                val pubResult = Validator.validateGamePublisher(newGameUiState.publisher)
                state = if(!pubResult.successful) newGameUiState.copy(pubErr = true) else newGameUiState.copy(pubErr = false)
            }

            is AddGameUiEvent.DeveloperChanged -> {
                val devResult = Validator.validateGameDeveloper(newGameUiState.developer)
                state = if(!devResult.successful) newGameUiState.copy(devErr = true) else newGameUiState.copy(devErr = false)
            }

            is AddGameUiEvent.PlatformChanged -> {
                val platResult = Validator.validateGamePlatform(newGameUiState.platform)
                state = if(!platResult.successful) newGameUiState.copy(platErr = true) else newGameUiState.copy(platErr = false)
            }

            is AddGameUiEvent.ImageChanged -> {
                val imgResult = Validator.validateGameImage(newGameUiState.image)
                state = if(!imgResult.successful) newGameUiState.copy(imgErr = true) else newGameUiState.copy(imgErr = false)
            }

            is AddGameUiEvent.RatingChanged -> {
                val ratingResult = Validator.validateGameRating(newGameUiState.rating)
                state = if(!ratingResult.successful) newGameUiState.copy(ratingErr = true) else newGameUiState.copy(ratingErr = false)
            }
            else -> {}
        }
